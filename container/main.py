...
def insertDistricts(stateId, districts):
  with database.batch() as batch:
    batch.insert_or_update(
        ...
        values=[(int(district["id"]), int(stateId), district["name"])
...
def insertSubDistricts(districtId, subDistricts):
  with database.batch() as batch:
    batch.insert_or_update(
        ...
        values=[(int(subDistrict["id"]), int(districtId), subDistrict["name"])
...
def insertVillages(subDistrictId, villages):
  with database.batch() as batch:
    batch.insert_or_update(
        ...
        values=[(int(village["id"]), int(subDistrictId), village["name"])
  ...
  if len(cards) > 0:
    with database.batch() as batch:
      batch.insert_or_update(
          ...
          values=[(villageId, card["sample"], card["village_grid"],
                   card["sr_no"]) for card in cards],
      )
...
def markCard(VillageId, Sample, SrNo, Ingested):
  database.run_in_transaction(lambda transaction: transaction.execute_update(
      ...
      param_types={
          ...
          "Ingested": spanner.param_types.BOOL
      },
  ))
...
def markVillage(VillageId, CardsLoaded):
  database.run_in_transaction(lambda transaction: transaction.execute_update(
      ...
      param_types={
          "VillageId": spanner.param_types.INT64,
          "CardsLoaded": spanner.param_types.BOOL
      },
  ))
      ...
      if len(districts) > 0:
        ...
        for district in districts:
          if int(district["id"]) >= DistrictId or DistrictId == -1:
            print(
                f"Loading and Ingesting SubDistricts for state {state['id']} and district {district['id']}"
            )
            subDistricts = await shc_dl.getSubDistricts(state["id"],
                                                        district["id"])
            subDistricts = sorted(
                subDistricts, key=lambda subDistrict: int(subDistrict["id"]))
            if len(subDistricts) > 0:
              insertSubDistricts(district["id"], subDistricts)
              for subDistrict in subDistricts:
                if int(
                    subDistrict["id"]) >= SubDistrictId or SubDistrictId == -1:
                  print(
                      f"Loading and Ingesting Villages for state {state['id']}, district {district['id']}, subDistrict {subDistrict['id']}"
                  )
                  villages = await shc_dl.getVillages(state["id"],
                                                      district["id"],
                                                      subDistrict["id"])
                  if len(villages) > 0:
                    insertVillages(subDistrict["id"], villages)
                  updateCheckpoint(1, state["id"], district["id"],
                                   subDistrict["id"])
                else:
...
async def ingestCards():
  ...
  if count < 1:
    ...
    print(
        f"At the moment at total of {count} villages are pending, scraping from {offset} {limit} villages with checkpoint start {start}"
    )
  with database.snapshot() as snapshot:
    results = snapshot.execute_sql(
    ...
    for row in results:
      ...
      StateId = row[3]
      print(
          f"Loading Card for {StateId} {DistrictId} {SubDistrictId} {VillageId}"
      )
      cards = await shc_dl.getCards(
          str(StateId), str(DistrictId), str(SubDistrictId), str(VillageId))
...
def get_village_view(VillageId):
  if village_view.get(VillageId):
    print('gotit\n')
    return village_view[VillageId]

  info = {}
  with database.snapshot() as snapshot:
    card_village_view = snapshot.execute_sql(
    ...
    for row in card_village_view:
      info["village"] = row[0]
      info["mandal_id"] = str(row[1])
      info["mandal"] = row[2]
      info["district_id"] = str(row[3])
      info["district"] = row[4]
      info["state_id"] = str(row[5])
      info["state"] = row[6]

  village_view[VillageId] = info
  return info
...
async def scrapeCards():
  ...
  if count < 1:
    ...
    print(
        f"At the moment a total of {count} cards is pending, scraping from {offset} {limit} cards"
    )
  with database.snapshot() as snapshot:
    results = snapshot.execute_sql(
...
def preprocess():
  india_shape_file = './testdata/india_shape.geojson'
  file = open(india_shape_file, 'r')
...
async def extractCards():
  """Extracts information from cards and stores in cards_info table in the database."""  
  start = time.time()

  cnt = 0
  india_shape = preprocess()

  num_cards_to_process = 1000
  offset = num_cards_to_process * int(TASK_INDEX) + random.randint(0, 100000)
  with database.snapshot() as snapshot:
    ingested_cards = snapshot.execute_sql(
        """SELECT VillageId, Sample, VillageGrid, SrNo, extract_attempt
          FROM Cards
          WHERE Ingested IS True
          AND not Extracted IS True
          AND extract_attempt < 5
          ORDER BY VillageId
          LIMIT @limit OFFSET @offset""",
        params={
            "limit": num_cards_to_process,
            "offset": offset
        },
        param_types={
            "limit": spanner.param_types.INT64,
    ...
    for row in ingested_cards:
      VillageId = str(row[0])
      Sample = row[1]
      VillageGrid = row[2]
      SrNo = str(row[3])
      extract_attempt = row[4]
      card = {
          "sample": Sample,
          "village_grid": VillageGrid,
          "sr_no": SrNo,
      ...
      card.update(get_village_view(VillageId))

      print(f"extracting card {card}\n")
      extractor_obj = CardExtractor(card, india_shape=india_shape)

      if extractor_obj.extract_card(False):
        cnt += 1
        print("card was extracted\n")
        markCardExtracted(int(VillageId), Sample, int(SrNo), True)

      curr = time.time()
      print(f'elapsed time: {curr-start}\ncard: {cnt}\n')


def markCardExtracted(VillageId, Sample, SrNo, Extracted):
  """Sets the Extracted column in cards table."""
  database.run_in_transaction(lambda transaction: transaction.execute_update(
      "UPDATE Cards "
      ...
      param_types={
          ...
          "Sample": spanner.param_types.STRING,
          "SrNo": spanner.param_types.INT64,
          "Extracted": spanner.param_types.BOOL
      },
  ))
